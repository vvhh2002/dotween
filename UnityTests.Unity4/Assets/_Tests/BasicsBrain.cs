...
public class BasicsBrain : BrainBase
{
	...
	void Start()
	{
		...
		for (int i = 0; i < targets.Length; ++i)
		{
			...
			case 0:
				default:
				tweens[i] = DOTween.To(()=> t.position, x=> t.position = x, new Vector3(0, 5f, 0), 1.5f).Relative().Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo);
				break;
			case 1:
				// Red cube (rotation)
				tweens[i] = DOTween.To(()=> t.rotation, x=> t.rotation = x, toRotation, 1.5f).Relative().Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false);
				break;
			case 2:
				tweens[i] = DOTween.To(()=> t.position, x=> t.position = x, new Vector3(0, 5f, 0), Plug.Vector3Options(true), 1.5f).Relative().Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo);
				break;
			}
		...
		// Additional tweens //////////////////////////
		// Float
		DOTween.To(()=> floatToTween, x=> floatToTween = x, 100, 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// Int
		DOTween.To(()=> intToTween, x=> intToTween = x, 100, 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// Uint
		DOTween.To(()=> uintToTween, x=> uintToTween = x, 50, 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// Vector2
		DOTween.To(()=> vector2toTween, x=> vector2toTween = x, new Vector2(50,100), 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// Vector4
		DOTween.To(()=> vector4toTween, x=> vector4toTween = x, new Vector4(50,100,150,200), 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// Rect
		DOTween.To(()=> rectToTween, x=> rectToTween = x, new Rect(10, 20, 50, 100), 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// RectOffset
		DOTween.To(()=> rectOffsetToTween, x=> rectOffsetToTween = x, new RectOffset(10, 20, 50, 100), 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// Color
		DOTween.To(()=> guiTexColor.color, x=> guiTexColor.color = x, Color.green, 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// Alpha
		DOTween.To(Plug.Alpha(()=> guiTexAlpha.color, x=> guiTexAlpha.color = x, 0f), 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// String
		DOTween.To(()=> stringToTween0, x=> stringToTween0 = x, "Hello I'm a new string!", 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// String
		DOTween.To(()=> stringToTween1, x=> stringToTween1 = x, "Hello I'm a new string!", 1.5f).Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
		// String (relative)
		DOTween.To(()=> stringToTween2, x=> stringToTween2 = x, "Hello I'm a new string!", 1.5f).Relative().Ease(EaseType.InOutQuad).Loops(loops, LoopType.Yoyo).AutoKill(false).Pause();
	}
